...
class DynamicStockModel(object):
    ...
    def dimension_check(self):
        ...
        try:
            DimReport = str(
                "<br><b> Checking dimensions of dynamic stock model " + self.name + "."
            )
            if self.t is not None:
                DimReport += str(
                    "Time vector is present with " + str(len(self.t)) + " years.<br>"
                )
            else:
                DimReport += str("Time vector is not present.<br>")
            if self.i is not None:
                DimReport += str(
                    "Inflow vector is present with " + str(len(self.i)) + " years.<br>"
                )
            else:
                DimReport += str("Inflow is not present.<br>")
            if self.s is not None:
                DimReport += str(
                    "Total stock is present with " + str(len(self.s)) + " years.<br>"
                )
            else:
                DimReport += str("Total stock is not present.<br>")
            if self.s_c is not None:
                DimReport += str(
                    "Stock by cohorts is present with "
                    + str(len(self.s_c))
                    + " years and "
                    + str(len(self.s_c[0]))
                    + " cohorts.<br>"
            ...
            if self.o is not None:
                DimReport += str(
                    "Total outflow is present with " + str(len(self.o)) + " years.<br>"
                )
            else:
                DimReport += str("Total outflow is not present.<br>")
            if self.o_c is not None:
                DimReport += str(
                    "Outflow by cohorts is present with "
                    + str(len(self.o_c))
                    + " years and "
                    + str(len(self.o_c[0]))
                    + " cohorts.<br>"
            ...
            if self.lt is not None:
                DimReport += str(
                    "Lifetime distribution is present with type "
                    + str(self.lt["Type"])
                    + ".<br>"
                )
            else:
            ...
            return str(
                "<br><b> Checking dimensions of dynamic stock model "
                + self.name
                + " failed."
            )
    ...
    def compute_outflow_pdf(self):
        ...
        if self.pdf is None:
            ...
            self.pdf[np.diag_indices(len(self.t))] = np.ones(
                len(self.t)
            ) - self.sf.diagonal(0)
            for m in range(0, len(self.t)):
                self.pdf[np.arange(m + 1, len(self.t)), m] = -1 * np.diff(
                    self.sf[np.arange(m, len(self.t)), m]
                )
            return self.pdf
        else:
            # pdf already exists
        ...
        if self.sf is None:
            ...
            if (
                ...
                for m in range(0, len(self.t)):  # cohort index
                    self.sf[m::, m] = np.multiply(
                        1, (np.arange(0, len(self.t) - m) < self.lt["Mean"][m])
                ...
                for m in range(0, len(self.t)):  # cohort index
                    if (
                        ...
                        self.sf[m::, m] = scipy.stats.foldnorm.sf(
                            np.arange(0, len(self.t) - m),
                            self.lt["Mean"][m] / self.lt["StdDev"][m],
                            0,
                            scale=self.lt["StdDev"][m],
                        )
                    ...
                    if (
                        ...
                        LT_LN = np.log(
                            ...
                            / np.sqrt(
                                ...
                                + self.lt["Mean"][m]
                                * self.lt["Mean"][m]
                                / (self.lt["StdDev"][m] * self.lt["StdDev"][m])
                            )
                        )
                        ...
                        self.sf[m::, m] = scipy.stats.lognorm.sf(
                            np.arange(0, len(self.t) - m),
                            s=SG_LN,
                            loc=0,
                            scale=np.exp(LT_LN),
    ...
    def compute_s_c_inflow_driven(self):
        ...
        if self.i is not None:
            if self.lt is not None:
                self.compute_sf()
                self.s_c = np.einsum(
                    "c,tc->tc", self.i, self.sf
                )  # See numpy's np.einsum for documentation.
    ...
    def compute_o_c_from_s_c(self):
        ...
        if self.s_c is not None:
            if self.o_c is None:
                ...
                self.o_c[np.diag_indices(len(self.t))] = self.i - np.diag(
                    self.s_c
            ...
            if len(InitialStock) == len(self.t):
                ...
                for Cohort in range(0, len(self.t)):
                    if self.sf[-1, Cohort] != 0:
                        ...
                        self.i[
                            Cohort
                        ] = 0  # Not possible with given lifetime distribution
                return self.i
            else:
    ...
    def compute_evolution_initialstock(self, InitialStock, SwitchTime):
        ...
        if self.lt is not None:
            ...
            ] = InitialStock  # Add initial stock to s_c
            self.s_c[SwitchTime::, 0:SwitchTime] = (
                np.tile(InitialStock.transpose(), (len(self.t) - SwitchTime, 1))
                * self.sf[SwitchTime::, 0:SwitchTime]
                / np.tile(Shares_Left, (len(self.t) - SwitchTime, 1))
            )
        return self.s_c
        ...
        if self.s is not None:
            if self.lt is not None:
                ...
                for m in range(
                    ...
                    if (
                        NegativeInflowCorrect is True
                    ):  # if the stock declines faster than according to the lifetime model, this option allows to extract additional stock items.
                        ...
                        if (
                            InflowTest < 0
                        ):  # if stock-driven model would yield negative inflow
                            ...
                            self.o_c[m, :] = self.o_c[m, :] + (
                                self.s_c[m, :] * Delta_percent
                            )  # increase outflow according to the lost fraction of the stock, based on Delta_c
                            self.s_c[m::, 0:m] = self.s_c[m::, 0:m] * (
                                1 - Delta_percent
                            )  # shrink future description of stock from previous age-cohorts by factor Delta_percent in current AND future years.
                        else:  # If no negative inflow would occur
                            if self.sf[m, m] != 0:  # Else, inflow is 0.
                                self.i[m] = (
                                    self.s[m] - self.s_c[m, :].sum()
                                ) / self.sf[
                                    m, m
                                ]  # allow for outflow during first year by rescaling with 1/sf[m,m]
    ...
    def compute_stock_driven_model_initialstock(
        self, InitialStock, SwitchTime, NegativeInflowCorrect=False
    ):
        ...
        ***
        ...
        if self.s is not None:
            if self.lt is not None:
                self.s_c = np.zeros((len(self.t), len(self.t)))
                self.s_c[
                    SwitchTime - 2, 0 : SwitchTime - 1
                ] = InitialStock  # assign initialstock to stock-by-cohort variable at END OF YEAR SwitchTime (here -1, because indexing starts at 0.).
                self.o_c = np.zeros((len(self.t), len(self.t)))
                self.i = np.zeros(len(self.t))
                ...
                if NegativeInflowCorrect is False:
                    for m in range(
                        SwitchTime - 1, len(self.t)
                    ):  # for all years m, starting at SwitchTime
                ...
                if NegativeInflowCorrect is True:
                    for m in range(
                        ...
                        if InflowTest < 0:
                            ...
                            # print('_')
                            self.o_c[m, :] = (
                                self.o_c[m, :] + (self.s_c[m, :] * Delta_percent).copy()
                            )  # increase outflow according to the lost fraction of the stock, based on Delta_c
                            self.s_c[m::, 0:m] = self.s_c[m::, 0:m] * (
                ...
                self.s[0 : SwitchTime - 1] = (
                    self.s_c[0 : SwitchTime - 1, :].sum(axis=1).copy()
                )
                return self.s_c, self.o_c, self.i
            else:
                # No lifetime distribution specified
    ...
    def compute_stock_driven_model_initialstock_typesplit(
        self, FutureStock, InitialStock, SFArrayCombined, TypeSplit
    ):
        """
        ...
        if self.s is not None:
            if self.lt is not None:
                ...
                s_cg = np.zeros(
                    (Nt0, Ntt, Ng)
                )  # stock for future years, all age-cohorts and product
                o_cg = np.zeros(
                    (Nt0, Ntt, Ng)
                )  # outflow by future years, all cohorts and products
                i_g = np.zeros((Ntt, Ng))  # inflow by product

                # Construct historic inflows
                for c in range(
                    0, SwitchTime
                ):  # for all historic age-cohorts til SwitchTime - 1:
                    for g in range(0, Ng):
                        if SFArrayCombined[SwitchTime - 1, c, g] != 0:
                            i_g[c, g] = (
                                InitialStock[c, g]
                                / SFArrayCombined[SwitchTime - 1, c, g]
                            )
                ...
                for t in range(
                    SwitchTime, Ntt
                ):  # for all years t, starting at SwitchTime
                    # 1) Compute stock at the end of the year:
                    s_cg[t - SwitchTime, :, :] = np.einsum(
                        "cg,cg->cg", i_g, SFArrayCombined[t, :, :]
                    )
                    # 2) Compute outflow during year t from previous age-cohorts:
                    if t == SwitchTime:
                        o_cg[t - SwitchTime, :, :] = (
                            InitialStock - s_cg[t - SwitchTime, :, :]
                        )
                    else:
                        o_cg[t - SwitchTime, :, :] = (
                            s_cg[t - SwitchTime - 1, :, :] - s_cg[t - SwitchTime, :, :]
                        )  # outflow table is filled row-wise, for each year t.
                    ...
                    for g in range(
                        0, Ng
                    ):  # Correct for share of inflow leaving during first year.
                        if (
                            SFArrayCombined[t, t, g] != 0
                        ):  # Else, inflow leaves within the same year and stock modelling is useless
                            i_g[t, g] = (
                                i_g[t, g] / SFArrayCombined[t, t, g]
                            )  # allow for outflow during first year by rescaling with 1/SF[t,t,g]
                        s_cg[t - SwitchTime, t, g] = (
                            i_g[t, g] * SFArrayCombined[t, t, g]
                        )
                        o_cg[t - SwitchTime, t, g] = i_g[t, g] * (
                            1 - SFArrayCombined[t, t, g]
                        )
    ...
    def compute_stock_driven_model_initialstock_typesplit_negativeinflowcorrect(
        self,
        SwitchTime,
        InitialStock,
        SFArrayCombined,
        ...
        if self.s is not None:
            if self.lt is not None:
                ...
                s_cg = np.zeros(
                    (Ntt, Ntt, Ng)
                )  # stock for future years, all age-cohorts and products
                o_cg = np.zeros(
                    (Ntt, Ntt, Ng)
                )  # outflow by future years, all cohorts and products
                ...
                NIC_Flags = np.zeros(
                    (Ntt, 1)
                )  # inflow flog for future years, will be set to calculated negative inflow value if negative inflow occurs and is corrected for.
                ...
                s_cg[:, 0:SwitchTime, :] = np.einsum(
                    "tcg,cg->tcg",
                    SFArrayCombined[:, 0:SwitchTime, :],
                    i_g[0:SwitchTime, :],
                )
                    ...
                    for m in range(
                        ...
                        for g in range(0, Ng):
                            if SFArrayCombined[m, m, g] != 0:  # Else, inflow is 0.
                                i_g[m, g] = (
                                    TypeSplit[m, g] * i0_test / SFArrayCombined[m, m, g]
                                )  # allow for outflow during first year by rescaling with 1/sf[m,m]
                                # NOTE: The stock-driven method may lead to negative inflows, if the stock development is in contradiction with the lifetime model.
                                # In such situations the lifetime assumption must be changed, either by directly using different lifetime values or by adjusting the outlfows,
                                # cf. the option NegativeInflowCorrect in the method compute_stock_driven_model.
                ...
                if NegativeInflowCorrect is True:
                    for m in range(
                        ...
                        if i0_test < 0:
                            ...
                            # adjust the entire stock AFTER year m as well, stock is lowered in year m, so future cohort survival also needs to decrease.
                            o_cg[m, :, :] = (
                                o_cg[m, :, :] + (s_cg[m, :, :] * Delta_percent).copy()
                            )  # increase outflow according to the lost fraction of the stock, based on Delta_c
                            s_cg[m::, 0:m, :] = s_cg[m::, 0:m, :] * (
                                1 - Delta_percent.copy()
                            )  # shrink future description of stock from previous age-cohorts by factor Delta_percent in current AND future years.
                            o_cg[m + 1 : :, :, :] = (
                                s_cg[m:-1, :, :] - s_cg[m + 1 : :, :, :]
                            )  # recalculate future outflows
